...
public class ProgramaNagusia {
    ...
    public static void liburuakIkusi() {
        ...
        liburu = Liburua.getLiburuSorta();

        System.out.println("LIBURUEN ZERRENDA");

        System.out.println("==========================");

        for (int i = 0; i < liburu.length; ++i) {
    ...
    public static void liburuenDatuGuztiakIkusi() {
        ...
        for (int i = 0; i < liburu.length; ++i) {
            ...
            System.out.printf("%-3s %-30s %-20s %4s %4s %-11s %1s %-6s %-15s \n", liburu[i + 1], liburu[i].getIzenburua(), liburu[i].getEgilea(), liburu[i].getOrriKopurua(), liburu[i].getUrtea(), liburu[i].getGaia(), liburu[i].getHizkuntza(), liburu[i].getPrezioa(), liburu[i].getKodea());
        }
    }
